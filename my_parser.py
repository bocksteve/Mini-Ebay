...
def writeItem(item):
    ...
    bidcount = getElementTextByTagNameNR(item, 'Number_of_Bids')
    description = getElementTextByTagNameNR(item, 'Description')

    writeLine(item_file, [itemID, name, currently, buy_price, first_bid, bidcount, started, ends, sellerID, description])
...
def parseXml(f):
    ...
    for i in range(Items.length):
        writeItem(Items[i])
        writeSeller(Items[i])
        writeCategories(Items[i])
    Bid = dom.getElementsByTagName('Bid')
    for b in range(Bid.length):
        writeBid(Bid[b], Bid[b].parentNode.parentNode.getAttribute('ItemID')) #may put current Itemid on all bids like cross product
